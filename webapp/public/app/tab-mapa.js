function Mapa() {
  ...
  const [mode, setMode] = useState("comunas");
  ...
  useEffect(() => {
    ...
    mapRef.current = map;
    fetch("data/comunas.geojson").then((r) => r.json()).then((gj) => {
      geoRef.current = gj;
      draw();
    });
